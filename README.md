...
- For mid-range values of $\rho$ in the range $10^{-3}-10^0$, the weight updates have a more stable development over boosting rounds when the losses $\boldsymbol{\ell}$ are normalized to sum to $1$. For larger values of $\rho > 1$, meaning that the worst-case scenario is searched in a large space of probability distributions, the weights $\boldsymbol{q}$ can become highly imbalanced, which reflects in a high imbalance of the losses $\boldsymbol{\ell}$ computed after the respective boosting round. The highly imbalanced losses may pose numerical problems to the respective following CVXPY optimization step if normalized to $1$. When this is the case and CVXPY fails to solve the inner optimization problem even though it would be feasible, a second attempt is performed using the rescaled loss vector $n \cdot \boldsymbol{\ell}$. If CVXPY still fails, the loss update is simply skipped in this boosting round.
...
3. The simulation and the ICU dataset analyses can be run independently of each other. For the ICU analyses, navigate to the folder `scripts/icumodels`. 
	...
	- After the desired models files have been run, the average losses over all boosting iterations and the weights after the last boosting iteration need to be collected for figure generation. Run the scripts `collect_weights.py` and `collect_losses.py` for this.
	- After losses and weights have been collected, the corresponding figures can be generated by running the files `modelfigures.py`, `rhofigures.py` and `weightfigures.py`. The generated figures are saved in the `results/figures/icu` folder.

4. For the simulation analyses, the code files `minority_group.py` and `concept_shift.py` in the `scripts/simulations` can be run independently of each other. The corresponding models are stored together with tables for the figures in the `results/models` folder. After running these files, run `minority_group_figures.py` and `concept_shift_figures.py` to generate the corresponding figures, which are then stored in `results/figures`. The codes of the simulation files also generate outputs of simple LightGBM models that are stored in the folder `exploration/simulations`.

To find reasonable parameter setting that optimize the trade-off between computation time and accuracy, we conducted several exploratory analyses on the ICU datasets. The most relevant results are stored in the form of trace plots of validation losses (that is, losses calculated on a validation set originating from the same source dataset) over boosting iterations. Exemplary results are stored in the folder `exploration/icu`. The codes for their generation are in the folder `scripts/icumodels/exploration` and can be run in a fashion similar to the codes used for the main models by running the corresponding .slurm files.
